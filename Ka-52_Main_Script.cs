...
void Init()
{
    ...
    DrawFlag(FlagLCD1);
    IMyTextPanel FlagLCD2 = (IMyTextPanel)GridTerminalSystem.GetBlockWithName("Flag LCD (Right)"); // Флаг декор
    DrawFlag(FlagLCD2);
}
...
void DrawFlag(IMyTextPanel LCDPanel) // Отрисовка флага РФ
{
    LCDPanel.ContentType = ContentType.SCRIPT;
    using (MySpriteDrawFrame Frame = LCDPanel.DrawFrame())
    {
        MySprite Sprite = new MySprite(SpriteType.TEXTURE, "SquareSimple", new Vector2(CenterScreen.X, 261f), new Vector2(512f, 113f), Color.Blue, "", TextAlignment.CENTER, 0f);
        Frame.Add(Sprite);
        Sprite = new MySprite(SpriteType.TEXTURE, "SquareSimple", new Vector2(CenterScreen.X, 148f), new Vector2(512f, 113f), Color.White, "", TextAlignment.CENTER, 0f);
        Frame.Add(Sprite);
        Sprite = new MySprite(SpriteType.TEXTURE, "SquareSimple", new Vector2(CenterScreen.X, 374f), new Vector2(512f, 113f), Color.Red, "", TextAlignment.CENTER, 0f);
        Frame.Add(Sprite);
    }
}
